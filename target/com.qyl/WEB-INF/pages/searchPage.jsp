    ...
    <script>
        ...
        function loadSpan() {
            ...
            if (${authority==1}) {
                ...
                $(".body .authority").css("display","none");
            }
        }
    </script>
</head>
    ...
    <div class="body content" style="margin: auto;float:none">
        ...
        <div class="body result" style="margin: auto;float:none">
            <table style=" border-collapse: collapse; text-align: center ;" border="1px" ; cellspacing="20px"
                ...
                <tr>
                    <td colspan="10">
                        <a href="fuzzyQueryAllBooks?pageNum=1&path=searchPage&typeName=${typeName}">首页</a><a
                            href="fuzzyQueryAllBooks?pageNum=${info.prePage}&path=searchPage&typeName=${typeName}"
                            contenteditable="${info.pageNum==1}"
                            style="outline: none">上一页</a>
                        <c:forEach items="${nums}" var="num">

                            <a href="fuzzyQueryAllBooks?pageNum=${info.prePage}&path=searchPage"
                               style="text-decoration:${num==info.pageNum?'none':'underline'}"
                               contenteditable="${num==info.pageNum}" style="outline: none">${num}</a>
                        </c:forEach>
                        <a
                                href="fuzzyQueryAllBooks?pageNum=${info.nextPage}&path=searchPage&typeName=${typeName}"
                                contenteditable="${info.pageNum==info.pages}" style="outline: none">下一页</a><a
                            href="fuzzyQueryAllBooks?pageNum=${info.pages}&path=searchPage&typeName=${typeName}">末页</a>
                    </td>
                </tr>
            </table>
        </div>
