        ...
        <div class="body result" style="margin: auto;float:none">
            <table style=" border-collapse: collapse; text-align: center ;" border="1px" ; cellspacing="20px"
                ...
                <tr>
                    ...
                    <th>出版商</th>
                    <th>类别</th>
                    <c:if test="${authority==1}"><th>借阅</th></c:if>
                    <c:if test="${authority==-1}"><th>修改</th><th>删除</th></c:if>
                </tr>
                <c:forEach items="${info.list}" var="book" varStatus="vs">
                    <tr>
                        ...
                        <td><a style="text-decoration: none;color: ${book.stock<=0?'black':'mediumblue'}"
                               href="getBookById?pageNum=${info.pageNum}&id=${book.id}&path=borrowBook" id="borrowBtn"
                               contenteditable="${book.stock <= 0}">借阅</a>
                        </td>
                        </c:if>
                        <c:if test="${authority==-1}">
                            <td><a style="text-decoration: none"
                                   href="getBookById?pageNum=${info.pageNum}&id=${book.id}&path=updateBook">修改</a>
                            </td>
                            <td><a style="text-decoration: none"
                                   href="deleteBookById?pageNum=${info.pageNum}&id=${book.id}&path=">删除</a>
                            </td>
                            </td>
                        </c:if>
                    </tr>
                ...
                <tr>
                    <td colspan="10">
                        <a href="
                        accurateQueryAllBooks?
                        &bookName=${info.list[0].bookName}
                        &bookEncrypt=${info.list[0].bookEncrypt}
                        &pubName=${info.list[0].pubName}
                        &typeName=${info.list[0].typeName}
                        &author=${info.list[0].author}
                        &pageNum=1">首页</a>
                        <a href="accurateQueryAllBooks?
                        &bookName=${info.list[0].bookName}
                        &bookEncrypt=${info.list[0].bookEncrypt}
                        &pubName=${info.list[0].pubName}
                        &typeName=${info.list[0].typeName}
                        &author=${info.list[0].author}
                        &pageNum=${info.prePage}"
                        ...
                        &typeName=${info.list[0].typeName}
                        &author=${info.list[0].author}
                        &pageNum=${info.nextPage}"
                                contenteditable="${info.pageNum==info.pages}" style="outline: none">下一页</a>
                        <a href="accurateQueryAllBooks?
                            accurateQueryAllBooks?
                        &bookName=${info.list[0].bookName}
                        &bookEncrypt=${info.list[0].bookEncrypt}
