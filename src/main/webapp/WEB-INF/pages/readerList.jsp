...
<div class="body">
    ...
    <div class="body result">
            <table style="margin: 5% auto; border-collapse: collapse; text-align: center ;" border="1px" ;
                ...
                <c:forEach items="${info.list}" var="reader" varStatus="vs">
                    <tr>
                        <td>${vs.index+1}</td>
                ...
                <tr>
                    <td colspan="9">
                        <a href="fuzzyQueryAllReaders?pageNum=1">首页</a><a
                            href="fuzzyQueryAllReaders?pageNum=${info.prePage}&typeName=${typeName}" contenteditable="${info.pageNum==1}">上一页</a>
                        <c:forEach items="${nums}" var="num">

                            <a href="fuzzyQueryAllReaders?pageNum=${num}"
                               style=" text-decoration:${num==info.pageNum?'none':'underline'};outline: none"
                               contenteditable="${num==info.pageNum}">${num}</a>
                        </c:forEach>
                        <a
                                href="fuzzyQueryAllReaders?pageNum=${info.nextPage}" contenteditable="${info.pageNum==1}">下一页</a><a
                            href="fuzzyQueryAllReaders?pageNum=${info.pages}">末页</a>
                    </td>
                </tr>
            </table>
    </div>
